...
src/
  ...
  components/
    App.jsx             — all React components and state logic (~790 lines)
  lib/
    db.ts               — @libsql/client singleton, schema init, helpers
    api.js              — fetch wrappers used by the React frontend
...
```sql
tasks    (id, col_key, text, due, tag, done, priority, completed_at, is_archived, origin_hue, created_at)
tags     (name)
settings (key, value)   -- currently stores: theme
```

**Key helper functions in `src/lib/db.ts`:**
- `getDb()` — returns initialised client; creates tables + seeds defaults on first call
- `rowToTask(row)` — maps DB row → frontend Task shape
- `sweepArchive()` — moves tasks with `done=1` and `completed_at < 30d ago` to `is_archived=1`
...
- **No seed/demo data** — board starts empty on fresh install
